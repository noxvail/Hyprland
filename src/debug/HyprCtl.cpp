...
std::string CHyprCtl::getWindowData(PHLWINDOW w, eHyprCtlOutputFormat format) {
    ...
    "xdgDescription": "{}"
            ...
            (g_pInputManager->isWindowInhibiting(w, false) ? "true" : "false"), NContentType::toString(w->getContentType()), escapeJSONStrings(w->xdgTag().value_or("")),
        ...
        return std::format(
            ...
            NContentType::toString(w->getContentType()), w->xdgTag().value_or(""), w->xdgDescription().value_or(""));
    }
}
