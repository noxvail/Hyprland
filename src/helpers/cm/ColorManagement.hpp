...
namespace NColorManagement {
    ...
    struct SImageDescription {
        ...
        bool needsLuminanceMapping(const SImageDescription& targetImageDescription) const {
            return fallbackLuminances && transferFunction == CM_TRANSFER_FUNCTION_EXT_LINEAR && luminances.reference != 0 &&
                luminances.reference != targetImageDescription.luminances.reference;
        }
    ...
    inline uint32_t   clampReferenceLuminance(float wanted, float min, float max, uint32_t fallback) {
        return static_cast<uint32_t>(max > min ? std::clamp(wanted, min, max) : fallback);
    }
