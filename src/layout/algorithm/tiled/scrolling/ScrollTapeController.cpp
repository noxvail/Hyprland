...
double CScrollTapeController::calculateCameraOffset(const CBox& usableArea, bool fullscreenOnOne) {
    ...
    if (maxExtent >= usablePrimary && *PFITMETHOD == 2)
        setOffset(std::clamp(m_offset, 0.0, maxExtent - usablePrimary));
    else if (maxExtent > usablePrimary && m_offset < 0.0 && *PFITMETHOD != 0)
        setOffset(0.0);
